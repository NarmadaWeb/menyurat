...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $stmt = $pdo->prepare("INSERT INTO incoming_mails (number, date_received, sender, subject, description) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$_POST['number'], $_POST['date_received'], $_POST['sender'], $_POST['subject'], $_POST['description']]);
    redirect(base_url('admin/surat-masuk/index.php'), 'Berhasil.');
}
...
<form method="POST" class="bg-white p-8 rounded-2xl border border-outline-variant space-y-4">
    ...
    <input type="date" name="date_received" required class="w-full border rounded-xl p-3">
    <input type="text" name="sender" required placeholder="Pengirim" class="w-full border rounded-xl p-3">
