<?php if (is_logged_in()): ?>
    ...
    <footer class="mt-auto border-t border-outline-variant bg-white py-6 px-6">
        <div class="flex justify-between items-center text-sm text-on-surface-variant">
            <p>&copy; <?= date('Y') ?> <?= get_village_profile()['nama'] ?>. All rights reserved.</p>
...
<?php else: ?>
    <footer class="bg-white border-t border-outline-variant py-12 mt-12">
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
                <h4 class="font-bold text-lg mb-4">E-Office <?= get_village_profile()['nama'] ?></h4>
                <p class="text-on-surface-variant text-sm"><?= get_village_profile()['alamat'] ?></p>
            </div>
            <div>
            ...
            <div>
                ...
                <p class="text-sm text-on-surface-variant">WA: <?= get_village_profile()['telepon'] ?></p>
